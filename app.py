...
def load(path: str) -> pd.DataFrame:
    ...
    df["Faixa Etária"] = df["Faixa Etária"].astype("str").apply(_faixa).astype("category")
...
with tabs[2]:
    ...
    def renda_disc_fig(df, lbl):
        ...
        # Usar todas as categorias de RENDA_ORDER para manter ordem completa
        df_rd["Renda"] = pd.Categorical(df_rd["Renda"], categories=RENDA_ORDER, ordered=True)
        df_rd = df_rd.sort_values("Renda")
        fig = px.line(df_rd, x="Renda", y="Média",
...
with tabs[4]:
    ...
    def raca_renda_fig(df, lbl):
        ...
        # Usar todas as categorias de RENDA_ORDER para manter ordem completa
        g["Renda Mensal Familiar"] = pd.Categorical(g["Renda Mensal Familiar"], categories=RENDA_ORDER, ordered=True)
        g = g.sort_values("Renda Mensal Familiar")
        fig = px.line(g, x="Renda Mensal Familiar", y="Média Geral", color="Cor/Raça",
                      markers=True, color_discrete_sequence=COLORWAY, title=lbl)
        fig.update_layout(xaxis_tickangle=-40)
        legend_right(fig)
        return fig
